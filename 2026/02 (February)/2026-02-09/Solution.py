...
# ---------------------------
# More optimal way by Claude

def ski_jump_medal_claude(distance_points: float, style_points: float, wind_comp: float, k_point_bonus: float) -> str:
    ...
    better_score_count = sum([1 for score in existing_scores if score > current_score])
    medals = ["Gold", "Silver", "Bronze", "No Medal"]

    return medals[min(better_score_count, 3)]

## Tests

assert ski_jump_medal_claude(125.0, 58.0, 0.0, 6.0) == "Gold"
assert ski_jump_medal_claude(119.0, 50.0, 1.0, 4.0) == "Bronze"
assert ski_jump_medal_claude(122.0, 52.0, -1.0, 4.0) == "Silver"
assert ski_jump_medal_claude(118.0, 50.5, -1.5, 4.0) == "No Medal"
assert ski_jump_medal_claude(124.0, 50.5, 2.0, 5.0) == "Gold"
assert ski_jump_medal_claude(119.0, 49.5, 0.0, 3.0) == "No Medal"